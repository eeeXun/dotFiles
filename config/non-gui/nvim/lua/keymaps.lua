...
-- quit when last window is nvim-tree
local function auto_close_nvim_tree()
    if #api.nvim_list_wins() == 1 and vim.bo.filetype == "NvimTree" then
        cmd.quit()
    end
end
map("n", "ZQ", function()
    cmd.quit({ bang = true })
    auto_close_nvim_tree()
end, "Quit without checking for changes")
map("n", "ZZ", function()
    cmd.xit()
    auto_close_nvim_tree()
end, "Write current file, if modified, and close the current window")
